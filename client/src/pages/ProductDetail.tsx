  ...
  return (
    <div className="product-page" dir={direction}>
      ...
      <footer className="product-footer product-footer--full">
        <div className="product-footer__statement"><Link href="/"><img src="/manus-storage/north-atelier-mark_c591b808.png" alt="" /> North Atelier</Link><p>{isArabic ? "قطع ترافقك في الطريق الطويل. مصنوعة بعناية لتكتسب شخصيتها مع الاستعمال." : isFrench ? "Des objets pour le long chemin. Finis à la main avec retenue, dessinés pour gagner du caractère avec l’usage." : "Objects for the long way home. Hand-finished with restraint, designed to gain character with use."}</p><button type="button" onClick={() => setLocale(locale === "ar" ? "en" : locale === "fr" ? "en" : "ar")}><Globe2 size={14} /> {isArabic ? "English" : isFrench ? "English" : "العربية"}</button></div>
        <div className="product-footer__links"><div><p>{isArabic ? "استكشاف" : isFrench ? "Explorer" : "Exploration"}</p><Link href="/#shop">{isArabic ? "تسوّق التشكيلة" : isFrench ? "Voir la sélection" : "Shop the edit"}</Link><Link href="/#story">{isArabic ? "ملاحظات الاستوديو" : isFrench ? "Notes de l’atelier" : "Studio notes"}</Link><Link href="/#journal">{isArabic ? "المجلة" : "Journal"}</Link><a href="mailto:studio@northatelier.example">{isArabic ? "تواصل معنا" : isFrench ? "Contacter l’atelier" : "Contact studio"}</a></div><div><p>{isArabic ? "عملي" : isFrench ? "Pratique" : "Practical"}</p><a href="#product-details">{isArabic ? "العناية والإصلاح" : isFrench ? "Soin et réparation" : "Care & repair"}</a><a href="#product-details">{isArabic ? "تفاصيل التوصيل" : isFrench ? "Détails de livraison" : "Delivery details"}</a><a href="#product-details">{copy.cashTitle}</a><Link href="/admin">{isArabic ? "لوحة الكتالوج" : isFrench ? "Gestion du catalogue" : "Catalog desk"}</Link></div></div>
        ...
        <div className="product-footer__base"><span>© 2026 North Atelier</span><span>{isArabic ? "صُنع ببطء. استُخدم جيداً." : isFrench ? "Fait lentement. Bien porté." : "Made slowly. Used well."}</span></div>
      </footer>
